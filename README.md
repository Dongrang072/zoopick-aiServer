...
| `POST` | `/cctv/enqueue` | CCTV 영상 분석 큐 등록 (WAS → AI) |
| `GET` | `/cctv/status/{video_id}` | 작업 상태 조회 (선택·디버그용) |
| `POST` | `/api/internal/cctv/progress` | (테스트용) 콜백 형태 로컬 검증 라우터 |
| `POST` | `/api/internal/cctv/detection` | 동상 |
| `POST` | `/api/internal/cctv/completed` | 동상 |
| `POST` | `/api/internal/cctv/failed` | 동상 |
